...
PROMPT="%B%(!.%{$fg[red]%}[%n@%m $fg[white]%1~$fg[red]]# %{$reset_color%}.%{$fg[green]%}[%n@%m %{$reset_color%}%B%1~%B%{$fg[green]%}]$%{$reset_color%} %b"
# Maia prompt
...
GIT_PROMPT_SYMBOL="%{$fg_bold[blue]%}±"                              # plus/minus     - clean repo
GIT_PROMPT_PREFIX="%{$fg_bold[green]%}[%{$reset_color%}"
GIT_PROMPT_SUFFIX="%{$fg_bold[green]%}]%{$reset_color%}"
GIT_PROMPT_AHEAD="%{$fg_bold[red]%}ANUM%{$reset_color%}"             # A"NUM"         - ahead by "NUM" commits
GIT_PROMPT_BEHIND="%{$fg_bold[cyan]%}BNUM%{$reset_color%}"           # B"NUM"         - behind by "NUM" commits
